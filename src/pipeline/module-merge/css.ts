...
const replaceAnimationNames = (value: string, keyframes: Map<string, string>) => {
  let output = value
...
const scopeCss = (
  ...
  const sourceCss = keyframePrefix ? scopeKeyframes(css, keyframePrefix) : css
  let output = ''
  let cursor = 0
...
export { scopeCss, scopeSelectorList }
